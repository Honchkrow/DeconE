...
plot_rare <- function(actual,
    ...
    # get real proportion
    p_true <- Deconer:::getInput(data = actual, name = "actual")
    ...
    if (figure == "barplot"){
        ...
        for (i in seq(length(label))) {
            this_label <- label[i]
            p_pred <- Deconer:::getInput(data = predicted[i], name = "predicted")
            p_pred <- Deconer:::reorder_df(df1 = p_true, df2 = p_pred)
        ...
        errbar_value <- apply(X = data1, MARGIN = 2, FUN = Deconer:::sde, type = errbar)
        ...
        errbar_value <- apply(X = data2, MARGIN = 2, FUN = Deconer:::sde, type = errbar)
        ...
        p_pred <- Deconer:::getInput(data = predicted, name = "predicted")
